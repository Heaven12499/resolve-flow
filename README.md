...
启动后，先在前端页面点击“同步知识库”。系统会将 3 份演示售后规则切分后，使用内置的中文 n-gram 向量化并写入 Milvus，不依赖 Hugging Face、BGE 或 PyTorch 下载。之后工单处理会按工单意图过滤物流/售后规则，并以相似度阈值过滤弱匹配内容；命中的规则会作为回复上下文，并在详情页展示“RAG规则依据”。

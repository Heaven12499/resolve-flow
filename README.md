...
- **安全与工程闭环**：Rule Engine 掌握退款和赔付门禁，结合 RBAC、人工审批、数据库权威任务队列、指数退避、执行租约及失败转人工。工单接入支持请求幂等，审批使用数据库锁和乐观版本控制，发券/退款会生成可追踪的业务动作执行记录。
...
| 验证项 | 结果 | 口径 |
| --- | --- | --- |
| 后端回归 | **Python 63/63、Java 26/26 passed** | 覆盖业务回归、不可变快照自主 Agent 循环、循环预算与补证暂停、AI 任务可靠性、工单接入幂等、审批并发控制、请求追踪和 Prometheus 指标端点；另有 2 项 Testcontainers 用例验证真实 MySQL 迁移与锁查询 |
...
- 订单仍使用演示数据；优惠券和退款通过可替换的业务动作适配器执行，本地实现会落库记录幂等键、请求、结果、外部流水和执行状态，但尚未接入真实支付或权益系统。
